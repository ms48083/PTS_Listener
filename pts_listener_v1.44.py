...
version = 'pts_listener.py version 1.44 2-Dec-14'
#
import socket
import base64
import MySQLdb
...
def parseSecureRemoval( byteStr ):
    ...
    if (HBarray[1] == 4): # new way
        ...
        HBarray.append(StrToLong(byteStr[16:]) + (ord(byteStr[22])<<32)) # 7 Card ID
        HBarray.append(ord(byteStr[20])) # 8 Status
...
def writeRawToLog( lognum, dataAry ):
    """
    Write the contents of the string array to the log file
    Uses dataAry[0] SystemNumber as part of the log file name
    """
    if (os.name=="nt"):
        filename = 'pts_' + str(lognum) + '.log'
    ...
    while 1:
            ...
            if (mypack[2]=="E"):	
                    ...
                    for a in tr:
                            print "%d " % a,
                    print
                    # writeRawToLog(tr[0], mypack) # log raw data
                    ...
                    for a in sr:
                            print "%d " % a,
                    print
                    # writeRawToLog(sr[0], mypack) # log raw data
