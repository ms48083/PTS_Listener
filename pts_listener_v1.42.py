...
version = 'pts_listener.py version 1.04 29-Jun-14'
#
import socket
import base64
import MySQLdb
...
def parseSecureRemoval( byteStr ):
	...
	# HBarray.append(StrToLong(byteStr[16:]) + (ord(byteStr[21])<<32)) # 7 Card ID
	HBarray.append( (ord(byteStr[16])<<32) + (ord(byteStr[17])<<24) + (ord(byteStr[18])<<16) + (ord(byteStr[19])<<8) + (ord(byteStr[21])) ) # 7 Card ID
	print "Secure "
	print ord(byteStr[16]),ord(byteStr[17]),ord(byteStr[18]),ord(byteStr[19]),ord(byteStr[21]), HBarray[7]
...
def getEventStationNames(dataAry, dbcursor):
    ...
    """
    if (dataAry[10] == 64 ):
        if (dataAry[11] == 1): # main door (status 64)
            print "ev64-1 "
            dataAry.append(getStationName(dataAry[0], dataAry[8], dbcursor))
            dataAry.append("")
