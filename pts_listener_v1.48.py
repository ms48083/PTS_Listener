...
# v1.46     30-Dec-2014  Implement auto-resync of transaction logs
# v1.47     14-Sep-2015  Add reload of lastcont parameters before closing database in case of external update
# v1.48     11-Nov-2015  Protection against old version remote com with new diverter; fix station name update from 99 due to diag data
version = 'pts_listener.py version 1.48 11-Nov-15'
#
import socket
import base64
import MySQLdb
...
def getLastCont(dataAry, dbcursor):
    """
    Read LastContTrans parameters from database into the data array
    """
    try:
        dbcursor.execute("SELECT * FROM parameters WHERE ParName = 'LastContTrans' ORDER BY ParVal1")
        # get the resultset as a tuple
        result = dbcursor.fetchmany(20) # return the last 20 records
        # iterate through resultset and capture locally
        for record in result:
            dataAry[record[2]] = record[3]
            # for x in record: print x,
            # print
            
    except:
        print "\nError reading parameters from db ", sys.exc_info()[0]
        
    return
    
def writeLastContIntoDb(lastCont, lastTouched, dbcursor):
    """
    Updates LastContTrans parameters to the database
    """
    try:
        for x in range(len(lastTouched)):
            if (lastTouched[x] == 1):
                # update record
                sql = "UPDATE `parameters` SET `ParVal2` = %s WHERE `ParName` = 'LastContTrans' AND `ParVal1` = %s" \
                        % (lastCont[x], x)
                print sql
                dbcursor.execute(sql)
                dbcursor.execute("COMMIT")
                lastTouched[x] = 0 # untouch
                print " System", x, " updated to", lastCont[x]
            
    except:
        print "\nError updating parameters to db ", sys.exc_info()[0]
...
def main():
    ...
    # define and initialize array for last continuous transaction
    lastCont = [0] * 21 # array of 0-20
    lastTouched = [0] * 21 # and an array saying which ones are updated

    # setup to block repeated card scans
    lastCard = 0
    lastTime = 0
    ...
    print "Loading Auto-Resync Value"
    getLastCont(lastCont, cursor)
    ...
    #for x in lastCont:
    #    print x,
    #sys.exit("Stopping execution just because")
    ...
    while 1:
            ...
            if (mypack[2]=="E"):	
                    ...
                    # update the lastContTrans parameter if difference is only 1
                    # tr[0] = system number; tr[5] = transaction number
                    if ((tr[5] - lastCont[tr[0]]) == 1):
                        lastCont[tr[0]] = tr[5] # new lastContTrans #
                        lastTouched[tr[0]] = 1 # touched
                    dbCloseTime = datetime.datetime.now() + dbCloseDelta
                    #cursor.close()
            elif (mypack[2]=="W"):
                    ...
                    print "SC ",
                    sr = parseSecureRemoval(mypack)
                    if ( (sr[7]==lastCard) and ((sr[6]-lastTime) <= 1) ):
                            print "Ignore repeat scan"
                    else:
                            for a in sr:
                                    print "%d " % a,
                            print
            ...
            if (dbOpened and (datetime.datetime.now() > dbCloseTime)):
                    writeLastContIntoDb(lastCont, lastTouched, cursor)
                    # re-read latest lastcont in case of an external re-sync
                    getLastCont(lastCont, cursor)
                    cursor.close
                    dbOpened = False
                    print "Db Closed"
